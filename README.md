...
- 이준우/김대원의 실시간 접속 여부 및 마지막 접속 시각 표시
...
이준우와 김대원은 프로필별 마지막 시각 하나가 아니라 브라우저 탭/세션별 접속 상태를 따로 기록합니다.

- 페이지 진입 즉시 현재 세션을 온라인으로 등록
- 15초마다 heartbeat 갱신
- 다른 사용자의 상태는 약 3초마다 다시 확인
- 최근 45초 안에 살아 있는 세션이 하나라도 있으면 `접속 중`
- 같은 사람이 여러 기기나 여러 탭에서 접속하면 모두 별도 세션으로 합산
- 방문자로 전환하면 해당 기기/네트워크의 기존 이준우·김대원 세션을 즉시 제거
- 이준우 ↔ 김대원으로 바꿔도 이전 프로필의 세션을 즉시 제거
- 탭을 숨기거나 페이지를 나갈 때 `sendBeacon`으로 즉시 퇴장 신호 전송
- 다시 화면으로 돌아오면 즉시 재접속 처리
- 브라우저 강제 종료처럼 퇴장 신호를 보내지 못한 경우에도 heartbeat가 끊기면 45초 뒤 자동으로 오프라인 판정
- 모든 사람이 나가 있는 상태에서 한 명이 들어오면 진입 직후 heartbeat를 보내므로 바로 온라인 처리
- 본인 화면은 heartbeat 응답을 받은 즉시 `접속 중`으로 표시
- 오프라인이면 마지막 heartbeat/퇴장 시각을 `마지막 접속`으로 표시
...
4자리 공유 ID, 프로필 매핑, 소유자별 공유 목록, 실시간 접속 상태를 저장하기 위해 Workers KV namespace가 필요합니다.
